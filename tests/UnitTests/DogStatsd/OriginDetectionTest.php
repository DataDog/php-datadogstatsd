...
{
    public function testCgroupV1() {
        $root = vfsStream::setup('proc');

        vfsStream::newFile("self/cgroup")
            ->withContent("4:blkio:/kubepods/burstable/podfd52ef25-a87d-11e9-9423-0800271a638e/8c046cb0b72cd4c99f51b5591cd5b095967f58ee003710a45280c28ee1a9c7fa\n")
            ->at($root);
        ...
        $originDetection->method('getFilePaths')->willReturn(array(
            "cgroupPath" => vfsStream::url("proc/self/cgroup")
        ));

        $containerId = $originDetection->getContainerID("", true);

        $this->assertSame("8c046cb0b72cd4c99f51b5591cd5b095967f58ee003710a45280c28ee1a9c7fa", $containerId, "Should have container id");
    }

    public function testParseContainerID() {
        $cases = [
            [
                'input' => "other_line
...
2:net_cls,net_prio:/kubepods/burstable/podfd52ef25-a87d-11e9-9423-0800271a638e/8c046cb0b72cd4c99f51b5591cd5b095967f58ee003710a45280c28ee1a9c7fa",
            ...
            [
                'input' => "10:hugetlb:/kubepods",
                'expected' => ""
            ],
            [
                'input' => "11:hugetlb:/ecs/55091c13-b8cf-4801-b527-f4601742204d/432624d2150b349fe35ba397284dea788c2bf66b885d14dfc1569b01890ca7da",
                'expected' => "432624d2150b349fe35ba397284dea788c2bf66b885d14dfc1569b01890ca7da"
            ],
            [
                'input' => "1:name=systemd:/docker/34dc0b5e626f2c5c4c5170e34b10e7654ce36f0fcd532739f4445baabea03376",
                'expected' => "34dc0b5e626f2c5c4c5170e34b10e7654ce36f0fcd532739f4445baabea03376"
            ],
            [
                'input' => "1:name=systemd:/uuid/34dc0b5e-626f-2c5c-4c51-70e34b10e765",
                'expected' => "34dc0b5e-626f-2c5c-4c51-70e34b10e765"
            ],
            [
                'input' => "1:name=systemd:/ecs/34dc0b5e626f2c5c4c5170e34b10e765-1234567890",
                'expected' => "34dc0b5e626f2c5c4c5170e34b10e765-1234567890",
            ],
            [
...
3:cpu:/invalid",
        ...
        foreach ($cases as $case) {
            $root = vfsStream::setup('proc');
            $procSelfCgroup = vfsStream::newFile("self/cgroup")
                ->withContent($case['input'])
                ->at($root);
...
',
            ...
            [
                'input' => '1258 1249 254:1 /docker/volumes/0919c2d87ec8ba99f3c85fdada5fe26eca73b2fce73a5974d6030f30bf91cbaf/_data/lib/containerd/io.containerd.grpc.v1.cri/sandboxes/ca30bb64884083e29b1dc08a1081dd2df123f13f045dadb64dc346e56c0b6871/hostname /etc/hostname rw,relatime - ext4 /dev/vda1 rw,discard',
                'expected' => ""
            ],
        ];
        ...
        foreach ($cases as $case) {
            ...
            $containerId = $originDetection->readMountInfo(vfsStream::url("proc/self/mountinfo"));
            $this->assertSame($case['expected'], $containerId, "Should have container id");
        }
    }

    public function testReadMountinfo() {
        $cid = "0cfa82bf3ab29da271548d6a044e95c948c6fd2f7578fb41833a44ca23da425f";
        $mountinfoContents = <<<EOT
...
EOT;
        ...
        $containerId = $originDetection->getContainerID("", true);
        $this->assertSame($cid, $containerId, "Should have container id");
    }

    public function testParsegroupControllerPath() {
        $cases = [
            [
        ...
        foreach ($cases as $case) {
            $this->assertSame($case['expected'], $originDetection->parseCgroupNodePath($case['content']), $case['name']);
        }
    }
    ...
    EOT,
        ...
        foreach ($tests as $test) {
            ...
            $originDetection->method('getFilePaths')->willReturn(array(
                "cgroupPath" => vfsStream::url("proc/self/cgroup"),
                "selfMountInfoPath" => vfsStream::url("proc/self/mountinfo"),
                "defaultCgroupMountPath" => vfsStream::url("sys/fs/cgroup"),
            ));

            $result = $originDetection->getCgroupInode(vfsStream::url("sys/fs/cgroup"), vfsStream::url("proc/self/cgroup"));
    ...
    public function testReadCIDOrInode() {
        $tests = [
            [
                ...
                'isHostCgroupNs' => true,
                'expectedResult' => '8c046cb0b72cd4c99f51b5591cd5b095967f58ee003710a45280c28ee1a9c7fa',
            ],
            [
